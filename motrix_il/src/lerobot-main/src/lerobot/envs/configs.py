...
def _is_libero_motrixsim_task(task: str | None) -> bool:
    return isinstance(task, str) and task.startswith("libero")
...
class AlohaEnv(EnvConfig):
    task: str | None = "AlohaTransferCube-v0"
    fps: int = 50
    ...
    def create_envs(self, n_envs: int, use_async_envs: bool = False):
        from lerobot.envs.motrixsim import create_motrixsim_envs

        env_cls = _make_vec_env_cls(use_async_envs, n_envs)
        return create_motrixsim_envs(
            task=self.task or "AlohaTransferCube-v0",
            n_envs=n_envs,
            env_cls=env_cls,
            episode_length=self.episode_length,
            suite_name=self.type,
        )


@EnvConfig.register_subclass("aloha_mujoco")
@dataclass
class AlohaMujocoEnv(AlohaEnv):
    task: str | None = "AlohaTransferCube-v0"
    ...
    def create_envs(self, n_envs: int, use_async_envs: bool = False):
        return EnvConfig.create_envs(self, n_envs=n_envs, use_async_envs=use_async_envs)
...
@EnvConfig.register_subclass("motrixsim")
@dataclass
class MotrixSimEnv(EnvConfig):
    task: str = "franka-lift-cube"
    fps: int = 100
    episode_length: int = 250
    features: dict[str, PolicyFeature] = field(
        default_factory=lambda: {
            ACTION: PolicyFeature(type=FeatureType.ACTION, shape=(8,)),
            "agent_pos": PolicyFeature(type=FeatureType.STATE, shape=(36,)),
        }
    )
    features_map: dict[str, str] = field(
        default_factory=lambda: {
    ...
    def __post_init__(self):
        if not _is_libero_motrixsim_task(self.task):
            return
    ...
    def create_envs(self, n_envs: int, use_async_envs: bool = True):
        if _is_libero_motrixsim_task(self.task):
            from lerobot.envs.libero_motrixsim import create_libero_motrixsim_envs

            env_cls = _make_vec_env_cls(use_async_envs, n_envs)
            return create_libero_motrixsim_envs(
                task=self.task, n_envs=n_envs, env_cls=env_cls, episode_length=self.episode_length
            )
...
@EnvConfig.register_subclass("libero_motrixsim")
@dataclass
