...
def eval_policy(
    ...
    video_write_errors: list[tuple[str, str]] = []
    ...
    def write_video_checked(video_path: Path, frames: np.ndarray, fps: int):
        try:
            ...
            logging.exception("Failed to write eval video to %s", video_path)
            video_write_errors.append((str(video_path), repr(exc)))
    ...
    for thread in threads:
        thread.join()
    if video_write_errors:
        failures = ", ".join(f"{path}: {err}" for path, err in video_write_errors)
        raise RuntimeError(f"Failed to write {len(video_write_errors)} eval video(s): {failures}")
